...
static int sdl_sound_init()
{
  ...
  as_desired.format   = SDL_AUDIO_S16;
  as_desired.channels = 2;

  sdl_sound.stream = SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &as_desired, NULL, NULL);
  if(!sdl_sound.stream) {
...
static void sdl_sound_set_enabled(int enabled)
{
  if (!sdl_sound.stream)
    return;
  ...
  if (enabled)
  {
    SDL_ResumeAudioStreamDevice(sdl_sound.stream);
  }
  else
  {
    SDL_PauseAudioStreamDevice(sdl_sound.stream);
    SDL_ClearAudioStream(sdl_sound.stream);
  }
}
...
static void sdl_sound_update(int enabled)
{
  ...
  if (enabled && sdl_sound.stream)
  {
    SDL_PutAudioStreamData(sdl_sound.stream, soundframe, size * sizeof(short));
  }
}
...
static void sdl_sound_close()
{
  ...
  {
    SDL_PauseAudioStreamDevice(sdl_sound.stream);
    SDL_DestroyAudioStream(sdl_sound.stream);
    sdl_sound.stream = NULL;
  }
}
...
static int sdl_video_init()
{
  ...
  const SDL_PixelFormat surface_format = SDL_PIXELFORMAT_RGB332;
#elif defined(USE_15BPP_RENDERING)
  const SDL_PixelFormat surface_format = SDL_PIXELFORMAT_RGB555;
#elif defined(USE_16BPP_RENDERING)
  const SDL_PixelFormat surface_format = SDL_PIXELFORMAT_RGB565;
#elif defined(USE_32BPP_RENDERING)
  const SDL_PixelFormat surface_format = SDL_PIXELFORMAT_RGB888;
#endif
  ...
  sdl_video.window = SDL_CreateWindow("Genesis Plus GX", VIDEO_WIDTH, VIDEO_HEIGHT, fullscreen ? SDL_WINDOW_FULLSCREEN : 0);
  if(!sdl_video.window) {
  ...
  if(!sdl_video.surf_screen) {
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error", "SDL Window surface creation failed", sdl_video.window);
    return 0;
  }
  sdl_video.surf_bitmap = SDL_CreateSurface(720, 576, surface_format);
  if(!sdl_video.surf_bitmap) {
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error", "SDL Bitmap surface creation failed", sdl_video.window);
    return 0;
  }
...
static void sdl_video_update()
{
  ...
  if(bitmap.viewport.changed & 1)
  {
    ...
    SDL_FillSurfaceRect(sdl_video.surf_screen, NULL, 0);
...
{
  if (sdl_video.surf_bitmap)
    SDL_DestroySurface(sdl_video.surf_bitmap);
  if (sdl_video.window)
...
struct {
  SDL_Semaphore* sem_sync;
  SDL_TimerID timer;
  unsigned ticks;
} sdl_sync;

static Uint32 SDLCALL sdl_sync_timer_callback(void *param, SDL_TimerID timerID, Uint32 interval)
{
  (void)param;
  (void)timerID;

  SDL_SignalSemaphore(sdl_sync.sem_sync);
  sdl_sync.ticks++;
  if (sdl_sync.ticks == (vdp_pal ? 50 : 20))
  {
    ...
    event.user.code = vdp_pal ? (sdl_video.frames_rendered / 3) : sdl_video.frames_rendered;
...
static int sdl_sync_init()
{
  ...
  {
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error", "SDL Semaphore creation failed", sdl_video.window);
    return 0;
  }
  sdl_sync.timer = 0;
  sdl_sync.ticks = 0;
...
static void sdl_sync_close()
{
  if(sdl_sync.timer)
  {
    SDL_RemoveTimer(sdl_sync.timer);
    sdl_sync.timer = 0;
  }
  if(sdl_sync.sem_sync)
  {
...
static int sdl_control_update(SDL_Keycode keystate)
{
    switch (keystate)
    {
      ...
      case SDLK_F4:
      {
        ...
        {
          use_sound ^= 1;
          sdl_sound_set_enabled(use_sound);
        }
        break;
      }
...
static int sdl_get_mouse_state(int *x, int *y)
{
  float fx = 0.0f;
  float fy = 0.0f;
  int state = (int)SDL_GetMouseState(x ? &fx : NULL, y ? &fy : NULL);
...
static int sdl_get_relative_mouse_state(int *x, int *y)
{
  float fx = 0.0f;
  float fy = 0.0f;
  int state = (int)SDL_GetRelativeMouseState(x ? &fx : NULL, y ? &fy : NULL);
...
int sdl_input_update(void)
{
  ...
  switch (input.dev[joynum])
  {
    case DEVICE_LIGHTGUN:
    {
      ...
      int x,y;
      int state = sdl_get_mouse_state(&x,&y);
    ...
    case DEVICE_PADDLE:
    {
      ...
      int x;
      int state = sdl_get_mouse_state(&x, NULL);
    ...
    case DEVICE_SPORTSPAD:
    {
      ...
      int x,y;
      int state = sdl_get_relative_mouse_state(&x,&y);
    ...
    case DEVICE_SMASH:
    {
      if(keystate[SDL_SCANCODE_KP_9])  input.pad[joynum] |= INPUT_SMASH_UP_RIGHT;
      if(keystate[SDL_SCANCODE_KP_8])  input.pad[joynum] |= INPUT_SMASH_UP;
      if(keystate[SDL_SCANCODE_KP_7])  input.pad[joynum] |= INPUT_SMASH_UP_LEFT;
      if(keystate[SDL_SCANCODE_KP_6])  input.pad[joynum] |= INPUT_SMASH_RIGHT;
      if(keystate[SDL_SCANCODE_KP_5])  input.pad[joynum] |= INPUT_SMASH_CENTER;
      if(keystate[SDL_SCANCODE_KP_4])  input.pad[joynum] |= INPUT_SMASH_LEFT;
      if(keystate[SDL_SCANCODE_KP_3])  input.pad[joynum] |= INPUT_SMASH_DOWN_RIGHT;
      if(keystate[SDL_SCANCODE_KP_2])  input.pad[joynum] |= INPUT_SMASH_DOWN;
      if(keystate[SDL_SCANCODE_KP_1])  input.pad[joynum] |= INPUT_SMASH_DOWN_LEFT;
      break;
    }
...
int main (int argc, char **argv)
{
  ...
  if(use_sound) sdl_sound_set_enabled(1);

  /* 3 frames = 50 ms (60hz) or 60 ms (50hz) */
  if(sdl_sync.sem_sync)
    sdl_sync.timer = SDL_AddTimer(vdp_pal ? 60 : 50, sdl_sync_timer_callback, NULL);
  ...
  while(running)
  {
    ...
    if (SDL_PollEvent(&event))
    {
      switch(event.type)
      {
        ...
        {
          running = sdl_control_update(event.key.key);
          break;
        }
      }
    }
    ...
    {
      SDL_WaitSemaphore(sdl_sync.sem_sync);
    }
  }
